...
class Portfolio:
    ...
    def add_or_update_investment(self, ticker: str, quantity: int, purchase_price: float) -> None:
        existing = self.find_investment(ticker)
        if existing:
            existing.quantity += quantity
            # We keep the purchase_price as-is; another strategy could be weighted average.
        else:
            self.holdings.append(Investment(ticker=ticker.upper(), quantity=quantity, purchase_price=purchase_price))
